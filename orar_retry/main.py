...
if last_dates_sem != dates or first_use:
    ...
    series_schedule = list(
        filter(lambda sch: True if sch.get("href").find("Orar" + year_no + series) != -1 else False, series_schedule))
    ...
    saved_dates = list(
        filter(lambda d: True if d.find('3' + year_no + group_no + series) != -1 else False, saved_dates))
    ...
    if to_download:
        schedule = requests.get(main_url + '/' + sem_links[i] + '/' + series_schedule[0].get("href"),
